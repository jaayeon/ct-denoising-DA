            ...
            for batch in dataloader[phase]:
                ...
                if phase == 'train':
                    ...
                    for _ in range(opt.n_d_train):
                        d_loss, gp_loss = net.d_loss(img,lbl,gp=True, return_losses=True)
                        d_loss.backward()
                        optimizer_d.step()
                    #generator, perceptual loss
                    optimizer_g.zero_grad()
                    net.generator.zero_grad()
                    g_loss, adv_loss, l_loss, p_loss = net.g_loss(img,lbl,perceptual=True, pixel_wise=True, return_losses=True)
                    g_loss.backward()
                    optimizer_g.step()
                ...
                status = [g_loss.item(), adv_loss.item(), l_loss.item(), p_loss.item(), d_loss.item(), psnr, npsnr]
