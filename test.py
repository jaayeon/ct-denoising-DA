...
if __name__ == '__main__':
    ...
    for di, test_dataloader in enumerate(dataloader['test']):
        ...
        for i, batch in enumerate(test_dataloader, 1):
            x, target, filename, domain_label = batch
            with torch.no_grad():
                input = {
                    'x': x,
                    'target': target,
        ...
        print("===> Test on {} - Noise Average Loss: {:.8f}, Noise Average PSNR: {:.8f}, Noise Average SSIM: {:.8f}, Average Loss: {:.8f}, Average PSNR: {:.8f}, Average SSIM: {:.8f}".format(
            opt.test_datasets[di], noise_avg_loss, noise_avg_psnr, noise_avg_ssim, avg_loss, avg_psnr, avg_ssim
        ))
        save_summary(opt, di, noise_avg_loss, noise_avg_psnr, noise_avg_ssim, avg_loss, avg_psnr, avg_ssim)