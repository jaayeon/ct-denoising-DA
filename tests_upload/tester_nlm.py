...
if __name__ == '__main__':
    ...
    img_dir = r'../../data/denoising/test/mayo/quarter_{}mm'.format(3) # quarter: img
    gt_img_dir = r'../../data/denoising/test/mayo/full_{}mm'.format(3) # full: gt_img
    device='cpu' #'cuda' for gpu

    img_list = glob.glob(os.path.join(img_dir, '**', '*.tiff'))
    gt_img_list = glob.glob(os.path.join(gt_img_dir, '**', '*.tiff'))
    test_result_dir = '../../data/denoising/test_result_DA/3mm_nlm'
    ...
    for img_idx, path in enumerate(zip(img_list,gt_img_list),1):
        if img_idx % 4 :
            ...
            print(f"estimated noise standard deviation = {input_sigma_est}")
            ...
            print('output_img type : {}'.format(np.type(output_img)))
