...
if __name__ == '__main__':
    ...
    if opt.thickness == 3:
        ...
        # real_std = 0.015403652024652922  #L067
    else : 
        real_std = 0.028929880168288947 #for all train set
        # real_std = 0.027103974722678868  #L067
    bp3mm = imread(bp3mm)
    ...
    for i in range(start_num, start_num+nums):
        bp3mm_2d = bp3mm[i,:,:]
        nbp3mm_2d = nbp3mm[i,:,:]
        noise = bp3mm_2d-nbp3mm_2d
        norm_noise = (noise-np.min(bp3mm_2d))/(np.max(bp3mm_2d)-np.min(bp3mm_2d))
