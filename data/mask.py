...
class Masker():
    ...
    def mask(self, X, i):
        ...
        # print('X.shape: ', X.shape) #torch.Size([1, 512, 512])
        # X = X.unsqueeze(0)
        # print('X.shpae: ', X.shape) #torch.Size([1, 1, 512, 512])

        phasex = i % self.grid_size
        phasey = (i // self.grid_size) % self.grid_size
        # mask = pixel_grid_mask(X.shape, self.grid_size, phasex, phasey)
        # print(X[0, 0].shape) # torch.Size([512])
        mask = pixel_grid_mask(X[0, 0].shape, self.grid_size, phasex, phasey)
        mask = mask.to(X.device)

        # print('mask: ', mask)
        # print('mask shape: ', mask.shape) # torch.Size([512, 512])
...
def pixel_grid_mask(shape, patch_size, phase_x, phase_y):
    ...
    # print('shape[-2:]: ', shape[-2:]) #torch.Size([512, 512])

    for i in range(shape[-2]):
        for j in range(shape[-1]):
            if (i % patch_size == phase_x and j % patch_size == phase_y):
                A[i, j] = 1
    # print('torch.tensor(A).shape : ', torch.tensor(A).shape) #torch.Size([512, 512])
...
def interpolate_mask(tensor, mask, mask_inv):
    ...
    # print('kernel.shape: ', kernel.shape) #torch.Size([1, 3, 3])
    # print('kernel', kernel)
